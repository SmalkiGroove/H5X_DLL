...
void MagicDisruptionFork();

int MagicDisruption_fork = 0x0097CFFD;
int MagicDisruption_return = 0x0097D004;

void MagicDisruption_init(pugi::xml_document& doc) {
    //assembly_patches.push_back({ PATCH_HOOK, MagicDisruption_fork, 7, MagicDisruptionFork, 0, 0, 0 });
...
__declspec(naked) void MagicDisruptionFork() {
    __asm
    {

        MAGIC_DISRUPTION_NEXT_LAST:
        mov edx, dword ptr [esi]
        mov ecx, esi
        call dword ptr [edx+0x74]
        jmp[MagicDisruption_return]
    }
}
